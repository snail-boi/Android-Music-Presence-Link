...
namespace AndroidMusicPresenceLink
{
    ...
    internal sealed partial class SettingsViewModel : ViewModelBase
    {
        ...
        private static bool AreConfigsEqual(MusicConfig left, MusicConfig right)
        {
            ...
            if (!string.Equals(left.Hotkeys.VolumeUpKeys ?? string.Empty, right.Hotkeys.VolumeUpKeys ?? string.Empty, StringComparison.OrdinalIgnoreCase)) return false;
            if (!string.Equals(left.Hotkeys.VolumeDownKeys ?? string.Empty, right.Hotkeys.VolumeDownKeys ?? string.Empty, StringComparison.OrdinalIgnoreCase)) return false;
            if (!string.Equals(left.Hotkeys.ToggleScrcpyKeys ?? string.Empty, right.Hotkeys.ToggleScrcpyKeys ?? string.Empty, StringComparison.OrdinalIgnoreCase)) return false;
            if (!string.Equals(left.Hotkeys.ToggleLyricsOverlayKeys ?? string.Empty, right.Hotkeys.ToggleLyricsOverlayKeys ?? string.Empty, StringComparison.OrdinalIgnoreCase)) return false;
            if (!string.Equals(left.Hotkeys.CopyTrackInfoKeys ?? string.Empty, right.Hotkeys.CopyTrackInfoKeys ?? string.Empty, StringComparison.OrdinalIgnoreCase)) return false;
            if (!string.Equals(left.Hotkeys.AudioQualityKeys ?? string.Empty, right.Hotkeys.AudioQualityKeys ?? string.Empty, StringComparison.OrdinalIgnoreCase)) return false;
