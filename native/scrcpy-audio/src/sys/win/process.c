...
sc_process_execute_p(const char *const argv[], HANDLE *handle, unsigned flags,
    ...
    if (!inherit_stdout && !inherit_stderr) {
        ...
        // This library runs inside a windowed host process that has no
        // console, so console children (adb) would otherwise allocate a
        // visible console window. CREATE_NO_WINDOW gives them an invisible
        // console instead, while keeping explicit pipe handles usable.
        // (Mutually exclusive with DETACHED_PROCESS.)
