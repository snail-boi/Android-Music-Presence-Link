...
static char *server_path_override;

bool
sc_server_set_server_path(const char *path) {
    free(server_path_override);
    server_path_override = path ? strdup(path) : NULL;
    return !path || server_path_override;
}

static char *
get_server_path(void) {
    if (server_path_override) {
        // Set explicitly (takes precedence over the environment variable)
        LOGD("Using server (explicit): %s", server_path_override);
        return strdup(server_path_override);
    }
