...
sc_audio_regulator_push(struct sc_audio_regulator *ar, const AVFrame *frame) {
    ...
    if (played) {
        ...
        uint32_t margin = ar->overbuffering_margin
                        ? ar->overbuffering_margin
                        : 60 * ar->sample_rate / 1000 /* 60 ms */;
        max_buffered_samples = ar->target_buffering * 11 / 10 + margin;
    } else {
...
sc_audio_regulator_init(struct sc_audio_regulator *ar, size_t sample_size,
    ...
    ar->overbuffering_margin = 0; // default (60 ms), may be set by the owner
