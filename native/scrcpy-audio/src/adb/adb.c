...
bool
sc_adb_set_executable(const char *path) {
    free(adb_executable);
    adb_executable = strdup(path);
    if (!adb_executable) {
        LOG_OOM();
        return false;
    }
    LOGD("Using adb (explicit): %s", adb_executable);
    return true;
}
...
sc_adb_init(void) {
    if (adb_executable) {
        // Already set explicitly via sc_adb_set_executable()
        return true;
    }
...
sc_adb_destroy(void) {
    free(adb_executable);
    // Reset so a future session re-resolves (or re-sets) the path instead of
