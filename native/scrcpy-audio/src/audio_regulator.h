...
struct sc_audio_regulator {
    ...
    // Extra margin (in samples) tolerated above the target buffering before
    // skipping samples. Must absorb the consumer's output buffer: a consumer
    // pulling in large chunks (e.g. WASAPI with tens of ms of latency) makes
    // the buffering level oscillate by that amount. 0 = default (60 ms).
    uint32_t overbuffering_margin;
